...
const forgotPassword = async (email: string) => {
  ...
  if (isUserExist.isActive === IsActive.DISABLED) {
    throw new AppError(StatusCodes.BAD_REQUEST, `User account is ${isUserExist.isActive}`);
  }
  ...
  const resetUILink = `${envVariables.FRONTEND_URL}/auth/reset-password?id=${isUserExist._id}&token=${resetToken}`;

  jobQueue.add("sendEmail", {
...
const verifyAccount = async (token: string) => {
  try {
    const decodedToken = jwt.verify(token, envVariables.VERIFY_ACCOUNT_SECRET) as JwtPayload;
    // console.log(decodedToken);

    const userFromDb = await User.findOne({ email: decodedToken.email });
    ...
    await User.findByIdAndUpdate(userFromDb._id, { isVerified: true });
